...
loss_NS = tf.abs(tf.reduce_mean(loss1 + loss2)) - lbd * tf.reduce_mean(tf.square(D_fake))
loss_NS_ksd = loss_NS + lbd_ksd * ksd

solver_NS_D = optimizer(learning_rate=lr_d).minimize(-loss_NS, var_list=theta_D)
solver_NS_G = optimizer(learning_rate=lr_g).minimize(loss_NS, var_list=theta_G)
solver_NS_G_ksd = optimizer(learning_rate=lr_g).minimize(loss_NS_ksd, var_list=theta_G)
...
for it in range(n_iter):
    ...
    for _ in range(n_D):
        _, loss_curr = sess.run([solver_NS_D, loss_NS],
    ...
    _, loss_curr = sess.run([solver_NS_G, loss_NS],
                            feed_dict={z: sample_z(mb_size, z_dim)})
